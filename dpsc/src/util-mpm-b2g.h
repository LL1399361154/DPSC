...
typedef struct B2gCtx_ {
    ...
    uint32_t (*Search)(struct MpmCtx_ *, struct MpmThreadCtx_ *, PrefilterRuleStore *, uint8_t *, uint16_t);

    /* we store our own multi byte search func ptr here for B2gSearch1 */
    uint32_t (*MBSearch2)(struct MpmCtx_ *, struct MpmThreadCtx_ *, PrefilterRuleStore *, uint8_t *, uint16_t);
    uint32_t (*MBSearch)(struct MpmCtx_ *, struct MpmThreadCtx_ *, PrefilterRuleStore *, uint8_t *, uint16_t);
} B2gCtx;
