...
static enum EngineMode g_engine_mode = ENGINE_MODE_IPS;
...
int PostConfLoadedSetup(SCInstance *suri)
{
    ...
    //设置模式匹配表
    MpmTableSetup();//多模式匹配 以注册AC匹配器为例，MpmTableSetup会调用MpmACRegister函数实现AC注册，函数内部其实只是填充mpm_table中对应 AC的那一项（mpm_table[MPM_AC]）的各个字段
    SpmTableSetup();//单模式匹配
    ...
    //设置checksum是否生效，checksum之前在ELK的统计数据中有很多误报，应该是因为route后五元组变了？
    ...
    RegisterAllModules();//注册了Suricata所支持的所有线程模块（Thread Module）
#ifdef HAVE_PLUGINS
    ...
    if (InitSignalHandler(suri) != TM_ECODE_OK)//初始化信号处理函数
        SCReturnInt(TM_ECODE_FAILED);
...
int InitGlobal(void) {
    ...
    ParseSizeInit();
    RunModeRegisterRunModes();//注册各种模式
