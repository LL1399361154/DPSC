...
void WmInitCtx (MpmCtx *);
void WmInitThreadCtx(MpmCtx *, MpmThreadCtx *);
void WmDestroyCtx(MpmCtx *);
void WmDestroyThreadCtx(MpmCtx *, MpmThreadCtx *);
...
int WmPreparePatterns(MpmCtx *);
uint32_t WmSearch(MpmCtx *mpm_ctx, MpmThreadCtx *mpm_thread_ctx, PrefilterRuleStore *, uint8_t *buf, uint16_t buflen);
uint32_t WmSearch1(MpmCtx *mpm_ctx, MpmThreadCtx *mpm_thread_ctx, PrefilterRuleStore *, uint8_t *buf, uint16_t buflen);
uint32_t WmSearch2Hash9(MpmCtx *mpm_ctx, MpmThreadCtx *mpm_thread_ctx, PrefilterRuleStore *, uint8_t *buf, uint16_t buflen);
uint32_t WmSearch2Hash12(MpmCtx *mpm_ctx, MpmThreadCtx *mpm_thread_ctx, PrefilterRuleStore *, uint8_t *buf, uint16_t buflen);
uint32_t WmSearch2Hash14(MpmCtx *mpm_ctx, MpmThreadCtx *mpm_thread_ctx, PrefilterRuleStore *, uint8_t *buf, uint16_t buflen);
uint32_t WmSearch2Hash15(MpmCtx *mpm_ctx, MpmThreadCtx *mpm_thread_ctx, PrefilterRuleStore *, uint8_t *buf, uint16_t buflen);
uint32_t WmSearch2Hash16(MpmCtx *mpm_ctx, MpmThreadCtx *mpm_thread_ctx, PrefilterRuleStore *, uint8_t *buf, uint16_t buflen);
...
inline uint32_t WmSearch(MpmCtx *mpm_ctx, MpmThreadCtx *mpm_thread_ctx, PrefilterRuleStore *pmq, uint8_t *buf, uint16_t buflen)
{
    WmCtx *ctx = (WmCtx *)mpm_ctx->ctx;
...
/* SCAN FUNCTIONS */
uint32_t WmSearch2Hash9(MpmCtx *mpm_ctx, MpmThreadCtx *mpm_thread_ctx, PrefilterRuleStore *pmq, uint8_t *buf, uint16_t buflen)
{
    WmCtx *ctx = (WmCtx *)mpm_ctx->ctx;
#ifdef WUMANBER_COUNTERS
    WmThreadCtx *tctx = (WmThreadCtx *)mpm_thread_ctx->ctx;
    ...
    if (pmq) {
        bitarray = alloca(pmq->rule_id_array_size);
        memset(bitarray, 0, pmq->rule_id_array_size);
    }
...
uint32_t WmSearch2Hash12(MpmCtx *mpm_ctx, MpmThreadCtx *mpm_thread_ctx, PrefilterRuleStore *pmq, uint8_t *buf, uint16_t buflen)
{
    WmCtx *ctx = (WmCtx *)mpm_ctx->ctx;
#ifdef WUMANBER_COUNTERS
    WmThreadCtx *tctx = (WmThreadCtx *)mpm_thread_ctx->ctx;
...
uint32_t WmSearch2Hash14(MpmCtx *mpm_ctx, MpmThreadCtx *mpm_thread_ctx, PrefilterRuleStore *pmq, uint8_t *buf, uint16_t buflen)
{
    WmCtx *ctx = (WmCtx *)mpm_ctx->ctx;
#ifdef WUMANBER_COUNTERS
    WmThreadCtx *tctx = (WmThreadCtx *)mpm_thread_ctx->ctx;
...
uint32_t WmSearch2Hash15(MpmCtx *mpm_ctx, MpmThreadCtx *mpm_thread_ctx, PrefilterRuleStore *pmq, uint8_t *buf, uint16_t buflen)
{
    WmCtx *ctx = (WmCtx *)mpm_ctx->ctx;
#ifdef WUMANBER_COUNTERS
    WmThreadCtx *tctx = (WmThreadCtx *)mpm_thread_ctx->ctx;
...
uint32_t WmSearch2Hash16(MpmCtx *mpm_ctx, MpmThreadCtx *mpm_thread_ctx, PrefilterRuleStore *pmq, uint8_t *buf, uint16_t buflen)
{
    WmCtx *ctx = (WmCtx *)mpm_ctx->ctx;
#ifdef WUMANBER_COUNTERS
    WmThreadCtx *tctx = (WmThreadCtx *)mpm_thread_ctx->ctx;
...
uint32_t WmSearch1(MpmCtx *mpm_ctx, MpmThreadCtx *mpm_thread_ctx, PrefilterRuleStore *pmq, uint8_t *buf, uint16_t buflen)
{
    WmCtx *ctx = (WmCtx *)mpm_ctx->ctx;
    uint8_t *bufmin = buf;
    uint8_t *bufend = buf + buflen - 1;
...
void WmInitThreadCtx(MpmCtx *mpm_ctx, MpmThreadCtx *mpm_thread_ctx)
{
...
void WmDestroyThreadCtx(MpmCtx *mpm_ctx, MpmThreadCtx *mpm_thread_ctx)
{
...
void MpmWuManberRegister (void)
{
    ...
    mpm_table[MPM_WUMANBER].InitThreadCtx = WmInitThreadCtx;
    mpm_table[MPM_WUMANBER].DestroyCtx = WmDestroyCtx;
    mpm_table[MPM_WUMANBER].DestroyThreadCtx = WmDestroyThreadCtx;
...
int WmTestThreadInitCtx01 (void)
{
    ...
    MpmInitCtx(&mpm_ctx, MPM_WUMANBER);
    WmInitThreadCtx(&mpm_ctx, &mpm_thread_ctx, 1);
    ...
    WmDestroyThreadCtx(&mpm_ctx, &mpm_thread_ctx);
    WmDestroyCtx(&mpm_ctx);
    return result;
#else
    return 1;
...
int WmTestSearch07 (void)
{
    ...
    WmPreparePatterns(&mpm_ctx);
    WmInitThreadCtx(&mpm_ctx, &mpm_thread_ctx, 2);
...
int WmTestSearch11 (void)
{
    ...
    WmPreparePatterns(&mpm_ctx);
    WmInitThreadCtx(&mpm_ctx, &mpm_thread_ctx, 3);
    WmCtx *ctx = (WmCtx *)mpm_ctx.ctx;
...
static int WmTestSearch22Hash9 (void)
{
    ...
    WmPreparePatterns(&mpm_ctx);
    WmInitThreadCtx(&mpm_ctx, &mpm_thread_ctx, 6 /* 6 patterns */);
